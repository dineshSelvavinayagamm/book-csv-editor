'use client';
import { getAccessRole, getAccessPrivilege, SecurityRolePrivilegeForm, SecurityRolePrivilegeUserCreate } from "@/api";
...
const SecurityRolePrivilegeCreate: React.FC = () => {
    ...
    const [privilegeOptions, setPrivilegeOptions] = useState<{ label: string; value: string }[]>([]);

    const [securityRolePrivilegeForm, setSecurityRolePrivilegeForm] = useState<SecurityRolePrivilegeForm>(
        formJson.reduce<SecurityRolePrivilegeForm>(
            (acc, field) => ({ ...acc, [field.name]: '' }),
            {} as SecurityRolePrivilegeForm,
        ),
    );
    ...
    const validateForm = useCallback(
        (formData: SecurityRolePrivilegeForm) => {
            const newErrors: Partial<SecurityRolePrivilegeForm> = {};
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <h2 className="text-2xl font-bold">Create New SecurityRolePrivilege</h2>

            {formJson.map((field) => {
                const fieldWithOptions = {
                    ...field,
                    options: field.name === 'securityRoleIdFKFld' ? roleOptions : field.name === 'securityPrivilegeIdFKFld' ? privilegeOptions : []
                };
